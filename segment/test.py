...
util.char2id, util.id2char = util.loadMap("char2id")
util.label2id, util.id2label = util.loadMap("label2id")
x_test, y_test = util.initFile(inputPath=test_path, seqMaxLen=num_steps)
...
with tf.Session(config=config) as sess:
	with tf.device(gpu_config):
		...
		with tf.variable_scope("model", reuse=None, initializer=initializer):
			model = BILSTM_CRF(num_chars=num_chars, num_classes=num_classes, util=util, num_steps=num_steps, embedding_matrix=embedding_matrix, is_training=False)
